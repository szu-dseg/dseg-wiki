...
???+ warning
    在开始编写一段内容之前，请查阅 [Issues](https://github.com/DSEG-wiki/DSEG-wiki/issues)，确认没有别人在做相同的工作之后，开个 [新 issue](https://github.com/DSEG-wiki/DSEG-wiki/issues/new) 记录待编写的内容。
...
参与 **DSEG Wiki** 的编写 **需要** 一个 GitHub 账号（可以前往 [GitHub 的账号注册页面](https://github.com/signup) 页面注册），但 **不需要** 高超的 GitHub 技巧，即使你是一名新手，只要按照下面所述的步骤操作，也能够 **非常出色** 地完成编辑。

???+ tip
    在你的更改被合并到 **DSEG Wiki** 的主仓库之前，你对 **DSEG Wiki** 的内容所作出的修改均不会出现在 **DSEG Wiki** 的主站上，所以无需担心你的修改会破坏 **DSEG Wiki** 上正在显示的内容。
    
    如果还是不放心，可以查看 [GitHub 的官方教程](https://skills.github.com/)。

#### 编辑单个页面内的内容

1.  在 **DSEG Wiki** 上找到对应页面；
...
4.  编写完成后滚动到页面下方，按照本文中 [commit 信息格式规范](#commit-信息格式规范) 填写 commit 信息，之后点击 **Propose changes** 按钮提交修改。点击按钮后，GitHub 会自动帮你创建一份 **DSEG Wiki** 仓库的分支，并将你的提交添加到这个分支仓库。
...
1.  打开 [DSEG-Wiki/DSEG-Wiki](https://github.com/DSEG-Wiki/DSEG-Wiki) 仓库，点击键盘上的<kbd>.</kbd>按钮（或者将 URL 中的 `github.com` 更改为 `github.dev`）[^ref2]，进入 GitHub 的网页版 VS Code 编辑器；
...
4.  提交后会在网页上方的中央弹出一个提示框，在第一次的提示框内填写标题，第二次的提示框内填写此提交要提交到的仓库内分支名称，之后右下角会弹出一个提示框，内容类似于 `Created Pull Request #1 for DSEG-Wiki/DSEG-Wiki.`，点击蓝字链接即可查看该 Pull Request。

#### 向 Pull Request 追加更改

1.  打开 [DSEG-Wiki 的 Pull Request 列表](https://github.com/DSEG-wiki/DSEG-wiki/pulls)，找到您提交的 Pull Request 并点击。
2.  Pull Request 页面的标题下方将会有一段例如 `<您的ID> wants to merge x commits into DSEG-wiki:master from <您的ID>:patch-1` 的文字，点击 `<您的ID>:patch-1` 部分。
...
在 Pull Request 页面下方可以找到测试页面，点击 netlify/DSEG-wiki/deploy-preview 一项的 Details 链接（如下图），可以进入自动构建的，由您变更后的页面供您预览。

![deploy\_preview](./images/deploy_preview.png)

### 对于目录和引用的变更

通常情况下，如果您需要添加一个新页面，或者修改已有页面在目录中的链接，您就需要对 [`mkdocs.yml`](https://github.com/DSEG-wiki/DSEG-wiki/blob/master/mkdocs.yml) 文件作出改动。
...
[`_redirects`](https://github.com/DSEG-wiki/DSEG-wiki/blob/master/docs/_redirects) 文件用于生成 [netlify 的配置](https://docs.netlify.com/routing/redirects/#syntax-for-the-redirects-file) 和 [用于跳转的文件](https://github.com/DSEG-wiki/DSEG-wiki/blob/master/scripts/gen_redirect.py)。
...
3.  请您仔细阅读 [贡献指南](https://github.com/DSEG-wiki/DSEG-wiki/blob/master/.github/CONTRIBUTING.md) 和 [社区公约](https://github.com/DSEG-wiki/DSEG-wiki/blob/master/CODE_OF_CONDUCT.md)，并在同意后勾选 PR 模板中的框，表示您同意了以上指南和公约。
...
2.  与此同时，在 [GitHub Actions](https://github.com/DSEG-wiki/DSEG-wiki/actions) 和 [Netlify](https://app.netlify.com/sites/DSEG-wiki) 上会运行两组测试，它们会把进度同步在 PR 页面的下方。GitHub Actions 主要用来确认 PR 中内容的修改不会影响到网站构建的进程；Netlify 用来把 PR 中的更新构建出来，方便 reviewer 审核（在测试完成后点击 Details 可以了解更多）；
